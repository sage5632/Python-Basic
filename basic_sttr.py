...
def string_oper():  # 문자열 연산
    ...
    # 슬라이싱 [시작경계:끝경계:간격]
    print("[::2] 슬라이싱", str1[::2]) # 처음부터 끝까지, 2 간격으로
    print("[::-1] 슬라이싱", str1[::-1])
...
def transform_methods():
    ...
    print("SWAPCASE:", s.swapcase()) # 대문자 <-> 소무자 변환
...
def search_methods():
    ...
    print("3rd Find:", s.find("Like", 20)) # 검색 결과 없으면 -1 반환
    ...
    # startswith, endswith로 검색 범위를 제한할수 있다.

    print("ftp의 6 ~ 20 영역이 ftp.으로 시작?", ftp.startswith("ftp.", 6, 20)) # 6 ~ 20 경계 영역이 ftp. 으로 시작?
...
def modify_reaplce_methods():
    ...
    s = "-------------Alice and the Heart Queen---------------"
    print("Strip :[", s.strip("-"), "]", sep="") # 앞뒤의 - 문자를 모두제거

    s = "I Love Java"
    print("원본:", s)
    print("REPLACE:", s.replace("Java", "Python")) # Java -> Python 바꿈
    print("원본:", s)
...
def align_methods():
    ...
    print("LJUST:[", s.ljust(60, "*"), "]", sep="") # 왼쪽정렬
    print("RJUST:[", s.rjust(60, "*"), "]", sep="") # 오른쪽 정렬

    print("ZFILL :", "1234".zfill(5)) # 5자리확보, 내용을 채운 후, 빈자리는 0으로 채움
    print("ZFILL :", "1234567890".zfill(5)) # 확보한 자리는 최소 공간, 자리수가 넘쳐도 내용은 잘리지 않음

def string_format():
    # 문자열 포맷 정리

    # C style 문자열 포맷
    # %s(문자열), %c, %d(정수형), %f(실수), %o, %x, %%(Literal %)
    fmt =  "%d개의 %s중에서 %d개를 먹었다."
    print(fmt % (10, '사과', 3))
    print("현재 이자율은 %2f%%입니다" % 1.2345) # %f 포맷은 부가적으로 소숫점 자리를 제한 할 수 있다
    ...
    print(fmt.format(eat=3, fruit="사과", total=11)) # 함수의 인자형태로 전달

    # 사전 객체 이용한 named parameter 연결
    data = {
        "total": 10, "fruit" : "포도", "eat": 5
    }
    print(fmt.format_map(data))

    # 가장 최신 스타일 : F - 문자열
    #   문자열 앞에 f or F
    #   변수의 이름 or 표현식 {} 안에 포함해서 값을 문자열로 출력
    total, fruit, eat = 10, 'apple', 3
    print(f"{total}개의 {fruit.upper()}을 {eat}개를 먹어서 {total - eat}개가 남았습니다")
