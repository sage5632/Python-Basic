...
print(sum_val(2,3)) # 고정 인수를 전달할때는 순서대로
print(incr(10)) # 기본 값이 있는 ㅇ니수는 전달하지 않으면 기본값이 선택
print(incr(10,2)) # 두번쨰 인수의 기본값 무시, 전달한 값을 할당
...
print(area(10,12)) # 인수 이름 명시 안함 -> 순언 순서대로 전달
...
# 순서 중요 : 고정 인수 -> 가변 인수 -> 사전 키워드 순
args_test( 10, 20, 30, 40, 50, option1="value1", option2="value2")

# 함수도 객체다
# 함수도 변수에 참조 할당되거나, 다른 함수의 인수로 전달 될수 있음
def plus(a, b):
    return a + b
...
print(calculator(5, 3, minus)) # 마이너스 함수를 외부에서 전달
