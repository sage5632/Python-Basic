def bool_ex(): #bool 자료형
    ...
    print("None:", bool(None)) # None : Java의 Null과 비슷 -> 아무것도 할당되지 않은 상태
...
def integer_ex(): #정수형
    ...
    # long 형의 저장크기인 4bit를 초과 하는 정수도 입력 가능
    i = 2 ** 2048
    print(i)
...
def float_ex():
    ...
    # is_integer : 타입 판별이 아니라 값의 형태가 정수형(소숫점이 아래 값이 없는지) 인지를 판별
    print(a.is_integer(), b.is_integer())

    # 소수점 포함, e, E 지수형태로 표현 가능
    c = 3e3 # 3 * 10 ** 3 == 3000.0
    d = -2E-5 # -2 * 10 ** -5 == -0.0002 같은 표현
...
def builtin_math_function(): # 내정 수치 함수
    print(abs(-1)) #절댓값
