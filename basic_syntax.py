...
def arith_oper(): # 산술 여산자 연습
    ...
    print(divmod(7, 3)) # 7을 3으로나눈 몫과 나머지
...
def complex_ex():
    ...
    print(complex(3, 4)) # 타입 함수

def rel_oper(): # 비교 연산자 연습
    print("===== 비교 연산(관계 연산)")
    print("1 > 3 ?", 1 > 3)
    print("1 < 3 ?", 1< 3)
    print("6 euqlas 9 ?", 6 == 9)
...
def variable_ex(): # 변수
        ...
        # 유니코드 이름의 변수도 사용가능 -> 권장x
        가격 = 12000
        print(가격 + 가격 * 0.1)

def assginment_ex(): #차원문
    print("===== 차원문")

    # 여러 변수를 한꺼번에 할당
    a , b = 3.5, 5.3 # 좌변의 변수의 갯수와 우변의 값이 갯수 일치해야 한다
    print(a, b)
...
def logical_oper():
    ...
    # 논리부정(not : True는 False , False는 True로)
    a, b = 20, 30
    print(not a < b) # a < b의 논리를 부정
    print(a < b and a != b) # a < b의 논리값과 a !=b 의 논리값의 논리곱
