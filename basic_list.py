...
def list_oper():
    ...
    # append vs extend
    copy.append(["Java", True, 3.14159])
    print(copy)
    copy.extend(["Java", True, 3.14159])
    print("Extend:", copy)
    ...
    # 슬라이싱 이용 치환
    # 메서드 이용보다 슬라이싱 이용 치환 방법 이용 권장
    lst = [1, 12, 123, 1234, 12345]
    print("원본:", lst)
    lst[0:2] = [10, 20]
    print(lst)
    # 슬라이싱을 이용한 삭제
    lst[0:2] = [] # 새 슬라이싱 범위에 빈 리스트를 할당
    print(lst)
    # 슬라이싱을 이용한 삽입
    ...
    print("MAX:", max(lst)) #최대값
    print("AVERGE:", sum(lst) / len(lst)) #평균
...
def list_method():
    ...
    print("원본:", copy)
    result = sorted(copy, key=str) # 키 함수를 str
...
def queue_ex():
    ...
    리스트를 활용한 QUEUE의 구현
        ...
        First Input First Out (FIFO) 자료형
    """
    queue = []
    queue.append(10)
    queue.append(20)
...
def loop():
    ...
    # 순차 자료형은 for ~ in 문으로 차례대로 요소를 전달 받을 수 있다 (별도 인덱스 변수는 없다)
    for word in words:
        print("WORD:", word)
