...
def read03():
    # readline 메서드 -> 전체 리스를 불러와서 리스트로 변환 제공
    f = open("./sample/multilines.txt", "r", encoding="UTF-8")
    line = f.readlines()
...
def pickle_dump_multi():
    # dump 메서드를 중복 실행하면 여러 객체를 dump
    with open("./sample/player.bin", "wb") as f:
        pickle.dump( { "baseball": 9 }, f, 1 ) # 프로토콜의 버전1 명시 가능
        pickle.dump( { "basketball": 5 }, f , pickle.HIGHEST_PROTOCOL ) # 가장 최신 버전의 프로토콜 사용
        pickle.dump( { "soccer": 11 }, f) # 프로토콜 버전 명시 안하면 -> 가장 최신 버전 프로토콜 사용

    print("중복 덤프 완료")

def pickle_load_multi():
    # 파일 내부에 몇 개의 ㅍ클 객체가 저장되어 있는지 확인 어렵다
    with open("./sample/player.bin", "rb") as f:
        # print(pickle.load(f))
        # print(pickle.load(f))
        # print(pickle.load(f))
